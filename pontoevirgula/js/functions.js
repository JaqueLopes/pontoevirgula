jQuery(function(){
	...
	//Cria uma janela de chat
	function add_janela(id, nome, status){
		//limita o número de janelas para 4 e as posiciona
		...
		//pega o id da conversa e mostra com quem vc está conversando
		var splitDados = id.split(':'); //(de:para)
		var id_user = Number(splitDados[1]); //seleciona qual é o usuário que vc esta falando (o para) 

		//abre a janela de chat com os dados do usuário 
				...
				jQuery.each(retorno, function(i, msg){
					if(jQuery('#janela_'+msg.janela_de).length > 0){
						if(userOnline == msg.id_de){
							...
							// acho q aq ta a img,ACHO
							jQuery('#janela_'+msg.janela_de+' .mensagens ul').append('<li id="'+msg.id+'"><div class="imgSmall"><img src="default.jpg'+msg.fotoUser[+'" /></div><p>'+msg.mensagem+'</p></li>');
						}
					}
				});
